...
contract DealSwapTemplate is Initializable, ReentrancyGuard {
    ...
    // 单边注入权限：仅 Factory 创建的 Deal
    modifier onlyDeal() {
        require(IDealFactory(factory).isDeal(msg.sender), "NOT_DEAL");
        _;
    }
    ...
    function mintOtherOnly() external nonReentrant onlyDeal returns (uint amountOther) {
        require(liquidityInited, "NOT_INIT");
